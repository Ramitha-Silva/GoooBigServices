	...
	public class UBLXML:IDisposable
	{
        ...
        private void SignDocument(string Directorypath, string xmldocument, string returnsignedxmldir, string returnnormalxmldir, Invoice inv, ref Result result)
        {
            try
            {
                ...
                string xmlfile = inv.SupplierParty.partyTaxScheme.CompanyID + "_" + inv.IssueDate.Replace("-", "") + "T" + inv.IssueTime.Replace(":", "") + "_" + RemoveNonAlphanumeric(inv.ID) + ".xml";
        ...
        //        DateTime.TryParseExact("2024-07-01", SettingsParams.allDatesFormats, CultureInfo.InvariantCulture, DateTimeStyles.None, out trailenddate);
		...
		private void GetICVElement(System.Guid icv, StringBuilder sb, ref string error)
		{
            ...
            try
            {
                ...
                foreach (AllowanceCharge allowance in allowanceCharges)
                {
                    if (allowance.Amount > 0)
                    {
                        if (allowance.ChargeIndicator)
                        {
                            chargetotalamount += Math.Round(allowance.Amount, 2, MidpointRounding.AwayFromZero);
                            // get allowance  vat 
                            chargetotalvat += Math.Round(allowance.Amount * (allowance.taxCategory.Percent / 100), 2, MidpointRounding.AwayFromZero);
                        }
                        else
                        {
                            allowancetotalamount += Math.Round(allowance.Amount, 2, MidpointRounding.AwayFromZero);
                            // get allowance  vat 
                            allowancechargetotalvat += Math.Round(allowance.Amount * (allowance.taxCategory.Percent / 100), 2, MidpointRounding.AwayFromZero);
                        }
                    }
                }
        ...
        private void RepairTaxCategory(InvoiceLine invline)
        {
            try
            {
                ...
                {
                    invline.item.classifiedTaxCategory.ID = invline.taxTotal.TaxSubtotal.taxCategory.ID.Trim();
                }
                if (string.IsNullOrEmpty(invline.taxTotal.TaxSubtotal.taxCategory.ID.Trim()))
                {
                    invline.taxTotal.TaxSubtotal.taxCategory.ID = invline.item.classifiedTaxCategory.ID.Trim();
                }
                // tax percentage
